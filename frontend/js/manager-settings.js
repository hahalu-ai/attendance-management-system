...
async function checkDeletionEligibility() {
    try {
        // Check for assigned contractors
        const response = await fetch(`${API_BASE_URL}/users/${currentUser.username}/contractors`);
        const data = await response.json();
        ...
        if (contractors.length > 0) {
            checkDiv.innerHTML = `
                <div class="warning-box">
                    ...
                    <p>You have ${contractors.length} contractor(s) assigned to you.</p>
                    <p>Please reassign them to another manager before deleting your account.</p>
                    <ul>
                        ${contractors.map(c => `<li>${c.display_name} (${c.username})</li>`).join('')}
                    </ul>
                </div>
            `;
